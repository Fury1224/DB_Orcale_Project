...
{
    public partial class professor상담조회 : Form
    {
        private string sql, ID;
        OleDbConnection conn;
        public professor상담조회(string sql, string ID)
        {
            InitializeComponent();
            this.sql = sql;
            this.ID = ID;
        ...
        private void professor상담조회_Load(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                cmd.CommandText = "select * from scott.상담 where 학번 in (select 학번 from scott.학생 where 지도교수 = '"
                    + ID + "')";
